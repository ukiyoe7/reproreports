...
assistentes <- dbGetQuery(con2,"SELECT CLICODIGO,FUNNOME ASSISTENTES FROM CLIEN C
                    LEFT JOIN FUNCIO F ON C.FUNCODIGO=F.FUNCODIGO
                     WHERE CLICLIENTE='S' AND C.FUNCODIGO IN (1843,1542,2158)")
...
PED AS (SELECT ID_PEDIDO,PEDDTBAIXA,P.CLICODIGO FROM PEDID P
    ...
    INNER JOIN CLI C ON P.CLICODIGO=C.CLICODIGO
     WHERE PEDSITPED<>'C' AND PEDDTBAIXA BETWEEN CAST(EXTRACT(YEAR FROM CURRENT_DATE) || '-01-01' AS DATE)
    AND
    DATEADD(-1 DAY TO CAST(CURRENT_DATE AS DATE))),
...
nsales <- apply(nsales,2,function(x) round(x,2)) %>% as.data.frame()
  ...
  data <- data %>% arrange(desc(.$YTD24)) %>% as.data.frame() %>% 
    rename_at(10:12,~ c(LASTMONTHTHISYEAR,THISMONTHLASTYEAR,CURRENTMONTH)) %>% inner_join(.,assistentes,by="CLICODIGO")
  
  corder <- c("ASSISTENTES","CLICODIGO","NOMEFANTASIA","CODGRUPO","GRUPO","SETOR","CIDADE", LASTMONTHTHISYEAR,CURRENTMONTH,"YTD23","YTD24","VAR24","STATUS","MEDIA23","MEDIA24","VARILUX DIGI","VARILUX TRAD","VARILUX XR","KODAK DIGITAL","KODAK TRAD","ACTUALITE","AVANCE","UZ+","IMAGEM DIGITAL","IMAGEM TRAD","INSIGNE","MARCAS PROPRIAS","EYEZEN","LA CRIZAL","LA KODAK","PROSING","PROSING POLY","VS ESSILOR","DESCTGERAL")
  
  data <- data %>% .[,corder] %>% mutate_all(~replace(., is.nan(.), NA)) %>% mutate(ASSISTENTES=str_trim(ASSISTENTES))
  
  
  ## get names to run loop
  
  NOMES_ASSISTENTES <- data.frame(ASSISTENTES=c("MARIANE FERREIRA MARQUES","CARLA FABIANA DE PINHO GODOY","CINTHIA PEREIRA NORONHA")) %>% mutate(ASSISTENTES=str_trim(ASSISTENTES))
  ...
  create_client_workbook <- function(client_name) {
    ...
    client_data <- data[data$ASSISTENTES == client_name, ]
    ...
    # Set column widths
    setColWidths(wb, sheet = "RANKING", cols = 1, widths = 33)
    
    setColWidths(wb, sheet = "RANKING", cols = 2, widths = 13)
    
    setColWidths(wb, sheet = "RANKING", cols = 3, widths = 40)
    
    setColWidths(wb, sheet = "RANKING", cols = 4, widths = 13)
    
    setColWidths(wb, sheet = "RANKING", cols = 5, widths = 30)
    
    setColWidths(wb, sheet = "RANKING", cols = 6, widths = 40)
    
    setColWidths(wb, sheet = "RANKING", cols = 7, widths = 25)
    
    setColWidths(wb, sheet = "RANKING", cols = 8, widths = 12)
    
    setColWidths(wb, sheet = "RANKING", cols = 9, widths = 12)
    
    setColWidths(wb, sheet = "RANKING", cols = 12:34, widths = 17)
    ...
    # Apply styles
    addStyle(wb, sheet = "RANKING", style = estiloNumerico, cols = c(8:11,14:34), rows = 1:3000, gridExpand = TRUE)
    ...
    # Save workbook
    saveWorkbook(wb, file = paste0("C:\\Users\\REPRO SANDRO\\OneDrive - Luxottica Group S.p.A (1)\\ATENDIMENTO\\RANKING_CLIENTES_ASSISTENTES_", gsub("^(\\S+\\s+\\S+).*", "\\1", client_name), ".xlsx"), overwrite = TRUE)
  }
  
  # Iterate over each client and create workbooks
  lapply(NOMES_ASSISTENTES$ASSISTENTES, create_client_workbook)
